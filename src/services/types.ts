namespace ts {
    /**
     * Created by the parser from tokens generated by the scanner which, among other things assign
     * them a `SyntaxKind`. In general, concrete Node types have the `kind` property of type
     * `ts.SyntaxKind`. TODO: See parser, scanner token, `SyntaxKind`
     *
     * Nodes of kind `SourceFile` act as root of any AST and its descendants know their `start` and
     * `end` position with respect the `SourceFile`'s text. See `getText(), `getFullText()`,
     * `getStart()`, `getFullStart()`, `getEnd()` and `getWidth()`.
     *
     * Many `Node` methods like `getChildren()` `getStart()`, `getText()`, etc. accept an optional
     * `SourceFile` argument. For performance reasons is recommended that users provide it in case
     * they already have a reference to the node's `SourceFile`. 
     *
     * The AST can be transversed top-down using `Node`'s `getChildren()` method or
     * `ts.forEachChild()` and down-up using the property `parent` that concrete Nodes implement.
     */
    export interface Node {
        /**
         * Returns the `SourceFile` node, root of the AST to which this node belongs.
         */
        getSourceFile(): SourceFile;        
        /**
         * Returns the length of the array returned by this node's `getChildren()` method
         */
        getChildCount(sourceFile?: SourceFile): number;
        /**
         * Returns child with given `index` in the array returned by `getChildren()`.
         */
        ...
        getEnd(): number;
        /**
         * Returns the length of this node in its source file's text as returned by `getText()`
         */
        ...
        getFullWidth(): number;
        /**
         * Return the length of this node's leading trivia text. "Trivia" is content like
         * whitespaces, comments, conflict markers, etc that is not relevant to understand the AST.
         */
        getLeadingTriviaWidth(sourceFile?: SourceFile): number;
        /**
         * Returns current text of given source file. In contrast to `getText`, `getFullText`
         * doesn't omit leading trivia.
         */
